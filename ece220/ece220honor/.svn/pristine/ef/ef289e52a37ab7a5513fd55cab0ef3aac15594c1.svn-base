...
find_nodes (locale_t* loc, vertex_set_t* vs, pyr_tree_t* p, int32_t nnum)
{
    ...
    if((4*nnum +1) >= p->n_nodes)
    {
      if(1 == in_range(loc, (p-> node)[nnum].x, (p-> node)[nnum].y_left))//check if it is in range
      {
        ...
        {
           vs->id = (int32_t*)malloc(10*sizeof(int32_t));//malloc 40 byte
          if(vs->id == NULL) //check if we fail malloc
          {
            return;
          }
          vs->id_array_size = 10; //change actual array size;
        }
        if(vs->count >= vs->id_array_size) //if our array full, realloc
          {
           vs->id = (int32_t*)realloc(vs->id, ((vs->id_array_size)+1)*sizeof(int32_t));//realloc more more
            {
              if(vs->id == NULL) //check if we fail realloc
              {
                return;
              }
            (vs->id_array_size)++; //change array size;
            }
          }
